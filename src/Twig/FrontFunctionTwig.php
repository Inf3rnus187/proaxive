...
class FrontFunctionTwig extends AbstractExtension
{
    public function getFunctions()
    {
        return [
            ...
            new TwigFunction('interType', [$this, 'getTypeIntervention']),
            new TwigFunction('getdata', [$this, 'getData']),
            new TwigFunction('breadcrumb', [$this, 'breadcrumb']),
            new TwigFunction('labelCustom', [$this, 'getLabelCustom']),
            new TwigFunction('roleicon', [$this, 'getRoleIcon'])
        ];
    }
    ...
    public function getActiveOrInactive(int $var){
        if ($var == 1){
            $html = '<span class="badge badge-light-success">Activé</span>';
        } elseif ($var == 0 or null){
            $html = '<span class="badge badge-light-danger">Désactivé</span>';
        }
        return $html;
    }
    ...
    /**
     * Permet d'afficher l'icon du role
     * @param $var
     * @return string
     */
    public function getRoleIcon($var){
        $html = '';
        if ($var == 1){
            $html = '<i class="ri-shield-star-fill"></i>';
        } elseif ($var == 2){
            $html = '<i class="ri-shield-fill"></i>';
        }
        return $html;
    }
     ...
     * @return int
     */
    public function getTypeIntervention(int $var = null){
        if ($var == null){
            $data = '<i>Non renseigné</i>';
        }
        if ($var == 1){
            $data = 'Dépannage';
        } elseif ($var == 2){
            $data = 'Visite Périodique';
        } elseif ($var == 3){
            $data = 'Préparation de poste';
        } elseif ($var == 4){
            $data = 'Assemblage';
        }
    ...
    public function getLabelCustom(string $bgcolor, ?string $txtcolor, string $title){
        if($txtcolor == null || empty($txtcolor)){
            $txtcolor = '#ffffff';
        }
