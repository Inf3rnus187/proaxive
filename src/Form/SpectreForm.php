...
class SpectreForm extends Form
{
     ...
     * @return string
     */
    public function tailDatetimeWithValue(string $name, string $label, string $value, $info = false)
    {
        $label = '<label class="form-label">' . $label .'</label>';
        $input = '<input type="text" name="'. $name .'" value="'.$value.'" class="form-input tail-datetime-field" />';
        if($info == true){
            $input.= '<small><i class="icofont-info-circle"></i> ' . $info . '</small>';
        }
        return $this->surroundInput($label . $input);
    ...
    public function inputHidden($name, $id){
        $html = '<input id="'. $id . '" name="' . $name . '" type="hidden" class="form-input" value="' . $this->getValue($name) . '" />';
        return $html;
    }
    ...
    public function inputColor($name, $label){
        $label = '<label class="form-label"><i class="ri-paint-fill"></i> ' . $label . '</label>';

        $html = '<input class="form-input" name="'.$name.'" data-jscolor="{}" type="text" value="' . $this->getValue($name) .  '" />';
    ...
    public function inputColorRgba($name, $label){
        $label = '<label class="form-label"><i class="ri-paint-fill"></i> ' . $label . '</label>';

        $html = '<input class="form-input" name="'.$name.'" data-jscolor="{required:false, format:\'rgba\'}" type="text" value="' . $this->getValue($name) .  '" />';
    ...
    public function inputColorRgbaWithVar($name, $label, $var){
        $label = '<label class="form-label"><i class="ri-paint-fill"></i> ' . $label . '</label>';

        $html = '<input class="form-input" name="'.$name.'" data-jscolor="{required:false, format:\'rgba\'}" type="text" value="' . $var .  '" />';

        return $this->surroundBlock($label . $html);
    }

    /**
     * COLORS With Variable
     */

    public function inputColorWithVar($name, $label, $var){
        $label = '<label class="form-label"><i class="ri-paint-fill"></i> ' . $label . '</label>';

        $html = '<input class="form-input jscolor" name="' . $name . '" type="text" value="' . $var .  '" />';
    ...
    public function checkbox($name, $label, $value = false){
        $label = '<label for="'.$value.'" class="checkbox">' . $label . '';
        $checkbox = '<input style="margin-right:1em;" type="checkbox" name="'.$name.'" value="1" id="'.$value.'" data-toggle="checkbox" class="switch"';
        ...
        return $this->switchSurround($checkbox . $label);
    }
    ...
    public function input($name, $label, $placeholder, $info = false, $id_input = false, $disable = false, $option = [], bool $binding = false, $attr = null){
        ...
        if($label == false){
            ...
            if($binding == true){
                $label = '<label class="form-label" for="' . $id_input . '">' . $label . ' <span style="color:red">*</span></label>';
            } else {
        ...
        if($type === 'textarea'){
            ...
            $input = '<textarea id="'.$id_input.'" class="form-input" name="'. $name .'" placeholder="'.$placeholder.'">' . $this->getValue($name) . '</textarea>';
            $input .= '<small>' . $info . '</small>';

        } else {

            $input = '<input class="form-input underlined '.$disable.'" type="'. $type .'" id="'. $id_input .'" name="'. $name .'" value="' . $this->getValue($name) . '" placeholder="'.$placeholder.'" '.$attr.' >';
            $input .= '<small>' . $info .'</small>';
        }
     ...
     * @return string
     */
    public function inputAjax($name, $label, $placeholder, $info = false){
        $label = '<label class="form-label" for="' . $name . '">' . $label . '</label>';
        $input = '<input id="txtHint" autocomplete="off"  onkeyup="searchCities(this.value)" class="form-input underlined" type="text" id="'. $name .'" name="'. $name .'" value="' . $this->getValue($name) . '" placeholder="'.$placeholder.'">';
        $input .= '<small>' . $info .'</small>';
       return $this->surroundBlock($label . $input);
    }
    ...
    public function inputWithVar($name, $label, $placeholder, $info = false, $option = [], $value){
        $type = isset($option['type']) ? $option['type'] : 'text';
        $label = '<label class="control-label">' . $label . '</label>';
        if($type === 'textarea'){

            $input = '<textarea class="form-control" name="'. $name .'" placeholder="'.$placeholder.'">' . $value. '</textarea>';
            $input .= '<small>' . $info .'</small>';

        } else {

            $input = '<input class="form-control underlined col-10-small-2" type="'. $type .'" name="'. $name .'" value="' . $value . '" placeholder="'.$placeholder.'">';
            $input .= '<small>' . $info .'</small>';
        }
    ...
    public function inputWithIcon($name, $label, $icon, $placeholder, $option = [], $info = false, $mdeditor = null){
        ...
        $input = '<div class="form-ic-comp"><i class="ri-'.$icon.'"></i></div><div class="input-st"><input class="form-control" type="'. $type .'" id="'. $name .'" name="'. $name .'" value="' . $this->getValue($name) . '" placeholder="'.$placeholder.'"></div>';
        $input .= '<small>' . $info . '</small>';
     ...
     * @return string
     */
    public function textareaDefault($name, string $label, string $placeholder, $info = false, $id = false)
    {
        $label = '<label class="form-label">' . $label . '</label>';
        $input = '<textarea id="'.$id.'" class="form-input" name="'. $name .'" placeholder="'.$placeholder.'">' . $this->getValue($name) . '</textarea>';
        $input .= '<small>' . $info . '</small>';
    ...
    {
        if($info == true){
            $info = '<small><i class="ri-information-fill"></i> ' . $info . '</small>';
        }
    ...
    {
        $label = '<label class="form-label">' . $label . '</label>';
        $input = '<input type="email" class="form-input underlined" value="'.$this->getValue($name).'" name="'. $name .'" placeholder="' . $placeholder .'" autocomplete="off">';
     ...
     * @return string
     */
    public function select($name, $label, $options, $info = false, $disabled = false){
        ...
        $input = '<div class="select"><select class="form-select" name="' . $name . '" ';
        if($disabled == true){
    ...
    public function submit(string $type, $name = false, int $id = NULL){
        ...
        return $this->surroundBlock('<button type="submit" name="'.$id.'" class="btn btn-'.$type.'"><i class="ri-checkbox-circle-line"></i> '.$name.'</button><div class="clear"></div>');
    ...
    public function submitDefault($title, string $type, $name = false){
        ...
        return $this->surroundDefault('<button type="submit" name="'.$name.'" class="btn btn-'.$type.'"><i class="ri-checkbox-circle-line"></i> '.$title.'</button>');
    ...
    public function submitSingleWithIcon($type, $name = false, $fa = false){
        ...
        if($name === false){$name = 'Confirmer';}
        if($fa === false){$fa = 'checkbox-circle-line';}
        return '<button type="submit" class="btn btn-'.$type.'"><i class="ri-'.$fa.'"></i> '.$name.'</button>';
    ...
    public function submitSingle($type, string $nameForm, $name = false, $fa = false){
        ...
        if($name === false){$name = 'Confirmer';}
        if($fa === false){$fa = 'checkbox-circle-line';}
        return '<button type="submit" name="'.$nameForm.'" class="btn btn-'.$type.'"><i class="ri-'.$fa.'"></i> '.$name.'</button>';
