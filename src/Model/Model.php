...
class Model{
    ...
    /**
     * Permet de compter le nombre de ligne d'une requête simple
     * @return int
     */
    ...
    /**
     * Permet de compter le nombre de ligne avec WHERE
     * @param int $id
     * @param string $where
     * @return int
     */
    public function countRowWhere(int $id, string $where){
        $rows = $this->query('SELECT id FROM ' . $this->model .' WHERE ' . $where .' = ' .$id);
        $total = count($rows);
        return $total;
    }
     ...
     * @return array
     */
    public function extractByIdAndKey($id, $key, $value){
        $records = $this->allById($id, $value);
        $return = [];
        foreach ($records as $v) {
            $return[$v->$key] = $v->$key;
    ...
    public function allDelete(){
        return $this->query("TRUNCATE TABLE {$this->model} ", null, true);
    ...
    /**
     * Permet de mettre à jour une date automatiquement
     * @param $idParent
     * @param $sql_part
     * @param $getId
     */
    public function updateDate($idParent, $sql_part, $getId){
        return $this->query("UPDATE {$this->model} SET $sql_part = NOW() WHERE $idParent = $getId");
    }
