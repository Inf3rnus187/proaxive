...
{
    protected $model = 'pe1x_equipments'; // Générer un nom de table différent
    ...
    public function allWithClientCategoryBrandsOS(){
        ...
        return $this->query("
            ...
            LEFT JOIN pe1x_clients as c ON c.id = e.client_id
            LEFT JOIN pe1x_category_equipments as ce ON e.category_equipment_id = ce.id
            LEFT JOIN pe1x_brands as b ON e.brand_id = b.id
            LEFT JOIN pe1x_operating_systems as os ON e.operating_systems_id = os.id
    ...
    public function findByIntervention($foreign)
    {
        ...
        *
        FROM ".$this->model." as e
        LEFT JOIN pe1x_interventions as i ON e.id = i.equipment_id
        LEFT JOIN pe1x_brands as b ON e.brand_id = b.id
        LEFT JOIN pe1x_operating_systems as os ON e.operating_systems_id = os.id
    ...
    public function selectWithCategoryInWork($limit) {
        return $this->query("
            SELECT 
            e.id as eid, name, inworkshop, content, e.created_at as edate, e.updated_at as eupdate,status,
            c.id as cid, fullname,
            ce.id as ceid, title, icon
            FROM ".$this->model." as e
            LEFT JOIN pe1x_clients as c ON c.id = e.client_id
