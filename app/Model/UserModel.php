...
class UserModel extends Model
{
    ...
    public function allWithRoles(){
        ...
        LEFT JOIN pe1x_roles as r ON u.roles_id = r.id
    ...
    public function allByLimit($limit) {
        return $this->query('SELECT *
        FROM ' . $this->model . ' as u
        LEFT JOIN pe1x_roles as r ON u.role_id = r.id
    ...
    /**
     * Récupère les infos nécessaire à la connexion
     * @param int $id
     * @param false $idParent
     * @return mixed
     */
    public function findForLogin(int $id, $idParent = false ){
        return $this->query("SELECT 
        id, password, fullname, pseudo, lastvisit, lastname, firstname,mail
        FROM " . $this->model ."
        WHERE id = ?", [$id], true);
    }

    /**
     * Recherche un mail dans la table utilisateur (Login)
     *
     * @param $mail
     * @return mixed
     */
    public function loginEmail($mail) {
        return $this->queryscan("SELECT * FROM " . $this->model . " as a WHERE mail = ? ", [$mail], true);
    }
